<li class="article-item {{ $article->hasImage() ? 'have-img' : '' }}">
  @if($article->hasImage())
    <a class="wrap-img" href="/article/{{ $article->id }}" target="{{ \Agent::isDeskTop()? '_blank':'_self' }}">
  ...
  <div class="content">
    ...
    <a class="title" target="{{ \Agent::isDeskTop()? '_blank':'_self' }}" href="/article/{{ $article->id }}">
        <span>{{ $article->title }}</span>
    </a>
    <a class="abstract" target="{{ \Agent::isDeskTop()? '_blank':'_self' }}" href="/article/{{ $article->id }}">
      {{ $article->description() }}
    </a>
    <div class="meta">
      @if($article->category)
      ...
      @endif
      <a target="{{ \Agent::isDeskTop()? '_blank':'_self' }}" href="/article/{{ $article->id }}">
        <i class="iconfont icon-liulan"></i> {{ $article->hits }}
      </a>
      <a target="{{ \Agent::isDeskTop()? '_blank':'_self' }}" href="/article/{{ $article->id }}/#comments">
