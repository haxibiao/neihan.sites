				...
				<div class="search-content">
					...
					<div class="note-list">
						...
						<li class="note-info"><a href="/category/{{ $category->id }}" class="avatar-category">
							<img src="{{ $category->logoUrl }}" alt=""></a>
							<follow
					            type="categories"
					            id="{{$category->id}}"
					            user-id="{{ user_id() }}"
							...
							<div class="title"><a href="/category/{{ $category->id }}" class="name">{{ $category->name }}</a></div>
							<div class="info"><p>收录了{{ $category->publishedWorks()->count() }}篇作品，{{ $category->count_follows }}人关注</p></div></li>
						@endforeach
					</div>
