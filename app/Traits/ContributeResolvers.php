...
trait ContributeResolvers
{
    ...
    {
        if ($user = checkUser()) {
            if (Contribute::getToDayCountByTypeAndId(self::AD_CONTRIBUTED_TYPE, self::AD_CONTRIBUTED_ID, $user) <= self::MAX_REWARD_CLICK_TIMES) {
                $contribute = Contribute::rewardUserContribute($user->id, self::AD_CONTRIBUTED_ID, self::REWARD_DRAW_AMOUNT,
...
//            超出30次限制，奖励双倍金币
            $goldAmount = Gold::REWARD_VIDEO_GOLD * 2;
