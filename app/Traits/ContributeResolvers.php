...
trait ContributeResolvers
{
    public function clickAD($rootValue, array $args, $context, $resolveInfo)
    {
        $user = checkUser();
//        兼容App老版本
        $contribute = Contribute::rewardUserContribute($user->id, self::AD_CONTRIBUTED_ID, self::AD_AMOUNT, self::AD_CONTRIBUTED_TYPE, "观看广告奖励");
        return $contribute;
    }
...
//            每天看激励视频获取贡献点限制10次
        if ($count < 10) {
            ...
            if ($isClick) {
                $contribute = Contribute::rewardUserContribute($user->id, self::VIDEO_CONTRIBUTED_ID, self::AD_VIDEO_AMOUNT, self::VIDEO_CONTRIBUTED_TYPE, "观看激励视频奖励");
            }
        } else {
//            超出10次限制，奖励双倍金币
