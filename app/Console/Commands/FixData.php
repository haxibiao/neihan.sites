<?php
    ...
    class FixData extends Command
    {
        ...
        public function hotRecommand()
        {
            Article::with(['comments', 'likes'])->whereNotNull('video_id')->whereStatus(1)->whereSubmit(1)->chunk(100,
                function ($articles) {
                    foreach ($articles as $article) {
                        $countComments = $article->comments()->count();
                        $countikes = $article->likes()->count();
                        if ($countComments >= 3 || $countikes >= 10) {
                            $this->info($article->count());
                            $article->is_hot = true;
        ...
        public function setVideoOffLine()
        {
            Article::whereNotNull('video_id')->chunkById(1000, function ($articles) {
                foreach ($articles as $article) {
                    ...
                    if (Str::contains($article->cover, 'http://cos-dongyundong.dianmoge.com/video/1.mp4.0_0.p0.jpg')) {
                        ...
                        $article->save();
                        $article->refresh();
                        $this->info('Article Id:' . $article->id . ' status:' . $article->status);
                    }
//                    $this->info('Article Id:' . $article->id . ' path:' . $article->video->path);
                }
        ...
        {
            $this->warn('批量将动态状态改为已经审核');

            Article::whereNotNull('video_id')->where([
                'submit' => Article::REVIEW_SUBMIT,
                'status' => Article::STATUS_REVIEW,
            ])->chunk(1000, function ($posts) {
                foreach ($posts as $post) {
                    $post->status = Article::STATUS_ONLINE;
                    $post->submit = Article::SUBMITTED_SUBMIT;
                    $post->save(['timestamps' => false]);

                    $post->refresh();
                    $this->info('Article Id:' . $post->id . ' status:' . $post->status);
                }
            });
        }

        public function articleReviewId()
        {
            $this->warn('修复文章的标题为视频文件名称的数据');

            Article::whereNull('review_id')->whereNotNull('video_id')->chunk(1000, function ($posts) use (&$count) {
                foreach ($posts as $post) {
                    $prefixNum = is_null($post->created_at) ? null : $post->created_at->format('Ymd');
                    $reviewId = Article::makeNewReviewId($prefixNum);
                    $post->review_id = $reviewId;
                    $post->save(['timestamps' => false]);
                    $count++;

                    $this->info('Article Id:' . $post->id . ' review_id:' . $post->review_id);
                }
            });
        }
