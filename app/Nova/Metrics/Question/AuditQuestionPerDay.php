...
class AuditQuestionPerDay extends Trend
{
    public $name = '每日审题数';
    ...
    public function calculate(Request $request)
    {
        ...
        $questions = Question::selectRaw(" distinct(date_format(reviewed_at,'%Y-%m-%d')) as daily,count(*) as count ")
            ->where('reviewed_at', '>=', now()->subDay($range - 1)->toDateString())
            ->groupBy('daily')
            ->get();
