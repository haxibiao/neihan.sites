...
class UpdatePost extends Action
{
        ...
        try {
            foreach ($models as $model) {
                ...
                if (isset($fields->tag1)) {
                    $tag = Tag::find($fields->tag1);
                    if ($tag) {
                        $model->tagByNames($tag->name);
                    }
                }
                if (isset($fields->tag2)) {
                    $tag = Tag::find($fields->tag2);
                    if ($tag) {
                        $model->tagByNames($tag->name);
                    }
                }
                if (isset($fields->tag3)) {
                    $tag = Tag::find($fields->tag3);
                    if ($tag) {
                        $model->tagByNames($tag->name);
                    }
                }
    ...
    {
        $data =  \App\Tag::query()->orderBy('count', 'DESC')->pluck('name', 'id')->toArray();
        return [
            ...
            //laravel 缺少复选框，先这样用
            SelectAutoComplete::make(_("合集1"), 'tag1')->options(
                $data
            ),

            SelectAutoComplete::make(_("合集2"), 'tag2')->options(
                $data
            ),

            SelectAutoComplete::make(_("合集2"), 'tag3')->options(
                $data
            ),
        ];
    }
