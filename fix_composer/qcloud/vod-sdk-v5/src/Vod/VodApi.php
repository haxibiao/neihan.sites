...
class VodApi
{
    ...
    {
        $result = self::deleteVodFile([
            'fileId' => $fileId,
            'isFlushCdn' => 1,
            'priority' => 1
        ]);
        if ($result == false) {
            return self::getErrRsp($id, 9000, "delete vod file failed");
        }
    ...
    private static function deleteVodFile(array $package)
    {
        $config = array('SecretId' => self::$secretId,
            'SecretKey'                => self::$secretKey,
            'RequestMethod'            => 'POST');
        ...
        $vod = QcloudApi::load(QcloudApi::MODULE_VOD, $config);
        $msg = "DeleteVodFile req package:" . json_encode($package);

        for ($retry = 0; $retry < 3; $retry++) {
            if ($retry > 0) {
                $msg = "DeleteVodFile retry at " . $retry;

            }
            $rsp = $vod->DeleteVodFile($package);
            if ($rsp == false) {
                $error = $vod->getError();
                $msg   = "DeleteVodFile failed, code: " . $error->getCode() . ", message: " . $error->getMessage() . "ext: " . var_export($error->getExt(), true);
        ...
        $msg = "DeleteVodFile|recv:" . json_encode($rsp);
        return $rsp;
    }

    public static function getFileInfo($fileName)
    {
        $result = self::describeVodPlayInfo([
            'fileName' => $fileName,
        ]);
        if ($result == false) {
            return self::getErrRsp($id, 9000, "get file info failed");
        }
    ...
    private static function describeVodPlayInfo(array $package)
    {
        $config = array('SecretId' => self::$secretId,
            'SecretKey'                => self::$secretKey,
            'RequestMethod'            => 'POST');
        ...
        $vod = QcloudApi::load(QcloudApi::MODULE_VOD, $config);
        $msg = "DescribeVodPlayInfo req package:" . json_encode($package);

        for ($retry = 0; $retry < 3; $retry++) {
            if ($retry > 0) {
                $msg = "DescribeVodPlayInfo retry at " . $retry;

            }
            $rsp = $vod->DescribeVodPlayInfo($package);
            if ($rsp == false) {
                $error = $vod->getError();
                $msg   = "DescribeVodPlayInfo failed, code: " . $error->getCode() . ", message: " . $error->getMessage() . "ext: " . var_export($error->getExt(), true);
        ...
        $msg = "DescribeVodPlayInfo|recv:" . json_encode($rsp);
        return $rsp;
    }
