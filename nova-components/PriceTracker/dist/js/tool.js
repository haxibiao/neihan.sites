...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    submit: function submit() {
      // 获取当前用户id
      var userId = Nova.config.userId;

      // 请求 api controller
      axios.post("/api/article/resolverDouyin", {
        params: {
        ...
        if (response.data.id) {
          window.open("/nova/resources/articles/" + response.data.id, "target");
        } else {
          alert(response.data);
        }
      }).catch(function (error) {
        alert("发布视频失败，联系 zengdawei@haxibiao.com 或者向其他的小哥求助");
      });
    }
  }
});
...
var render = function() {
  ...
  return _c(
    ...
    {
      staticClass:
        "app min-h-screen min-v-screen p-8 bg-grey-lightest font-sans"
    },
    [
      _c("div", { staticClass: "row sm:flex" }, [
        _c("div", { staticClass: "col sm:w-1/2" }, [
          _c(
            "div",
            { staticClass: "box border rounded flex flex-col shadow bg-white" },
            [
              _vm._m(0),
              _vm._v(" "),
              _c("textarea", {
                ...
                staticClass: "text-grey-darkest flex-1 p-2 m-1 bg-transparent",
                attrs: {
...
var staticRenderFns = [
  function() {
    ...
    return _c(
      "div",
      { staticClass: "box__title bg-grey-lighter px-3 py-2 border-b" },
      [
        _c("h3", { staticClass: "text-sm text-grey-darker font-medium" }, [
          _vm._v("导入抖音视频")
        ])
      ]
    )
